...
namespace DottedLine
{
    public class DottedLine : MonoBehaviour
    {
        ...
        private GameObject CreateUIDot()
        {
            var go = new GameObject();
            //go.transform.localScale = Vector3.one * Size;
            go.transform.parent = REF.CombatUI._steeringWheelScript._chainParent;
        ...
        private void GrowUIPool()
        {
            for (int i = 0; i < 500; i++)
            {
                ...
                dot.transform.SetParent(REF.CombatUI._steeringWheelScript._chainParent); //add all the created bullets to the pool so they aren't loose in the scene
                AddToUIPool(dot);
            }
        }
