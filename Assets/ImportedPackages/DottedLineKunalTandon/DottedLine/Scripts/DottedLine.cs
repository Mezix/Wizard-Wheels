...
namespace DottedLine
{
    public class DottedLine : MonoBehaviour
    {
        ...
        private GameObject CreateUIDot()
        {
            var go = new GameObject();
            //go.transform.localScale = Vector3.one * Size;
            go.transform.parent = Ref.UI._steeringWheelScript._dottedLinesParent;
        ...
        private void GrowUIPool()
        {
            for (int i = 0; i < 500; i++)
            {
                ...
                dot.transform.SetParent(Ref.UI._steeringWheelScript._dottedLinesParent); //add all the created bullets to the pool so they aren't loose in the scene
                AddToUIPool(dot);
            }
        }
